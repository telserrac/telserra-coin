...
CzTSCTracker::CzTSCTracker(std::string strWalletFile)
{
...
CzTSCTracker::~CzTSCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzTSCTracker::Init()
{
...
bool CzTSCTracker::Archive(CMintMeta& meta)
{
...
bool CzTSCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzTSCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzTSCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzTSCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzTSCTracker::GetSerialHashes()
{
...
CAmount CzTSCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzTSCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzTSCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzTSCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzTSCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzTSCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzTSCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzTSCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzTSCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzTSCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzTSCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTSCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTSCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzTSCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzTSCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzTSCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzTSCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dztsc from DB\n", __func__, listDeterministicDB.size());
    }
