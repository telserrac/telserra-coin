...
CzTSCWallet::CzTSCWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTSCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTSCSeed_deprecated()) {
                    LogPrintf("%s: Updated zTSC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzTSCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTSCCount(nCountLastUsed);
    else if (!walletdb.ReadZTSCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTSCWallet::Lock()
{
    seedMaster = 0;
}

void CzTSCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTSCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTSC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTSCWallet::LoadMintPoolFromDB()
{
...
void CzTSCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTSCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTSCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->ztscTracker->HasPubcoinHash(pMint.first)) {
...
bool CzTSCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTSC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to ztscTracker which also adds to database
    pwalletMain->ztscTracker->Add(dMint, true);
...
void CzTSCWallet::SeedToZTSC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzTSCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTSCWallet::GenerateDeterministicZTSC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTSCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTSCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
