...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTelSerraCoinAmount"))
        settings.setValue("nAnonymizeTelSerraCoinAmount", 1000);

    nAnonymizeTelSerraCoinAmount = settings.value("nAnonymizeTelSerraCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTelSerraCoinAmount"))
        SoftSetArg("-anonymizetelserracoinamount", settings.value("nAnonymizeTelSerraCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in telserracoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTelSerraCoinAmount:
            return QVariant(nAnonymizeTelSerraCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTelSerraCoinAmount:
            nAnonymizeTelSerraCoinAmount = value.toInt();
            settings.setValue("nAnonymizeTelSerraCoinAmount", nAnonymizeTelSerraCoinAmount);
            emit anonymizeTelSerraCoinAmountChanged(nAnonymizeTelSerraCoinAmount);
            break;
        case CoinControlFeatures:
