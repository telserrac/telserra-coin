...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->ztscTracker->HasMintTx(hash))) {
            //zTSC stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZTSC;
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by TelSerraCoin Address
...
bool IsZTSCType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZTSC:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZTSC || type == TransactionRecord::MNReward) {
