...
#define URI_SCHEME "telserracoin"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a TelSerraCoin address (e.g. %1)").arg("67VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no TelSerraCoin: URI
...
{
    // Convert telserracoin:// to telserracoin:
    //
    //    Cannot handle this later, because telserracoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open telserracoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TelSerraCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for TelSerraCoin.lnk
...
{
    return GetAutostartDir() / "telserracoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a telserracoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=TelSerraCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the telserracoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add telserracoin app to startup item list
