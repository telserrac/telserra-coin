...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered telserracoin address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal telserracoin:URI (telserracoin:XyZ...?message=example)
            {
...
static void NotifyzTSCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzTSCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzTSCReset.connect(boost::bind(NotifyzTSCReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzTSCReset.disconnect(boost::bind(NotifyzTSCReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->ztscTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
