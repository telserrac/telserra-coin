...
// The zTSC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zTSC is a hash of the serial
...
{
    //Create an output returning the zTSC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZTSCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zTSC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zTSC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZTSCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zTSC output", __func__);
...
{
    CzTSCTracker* ztscTracker = pwallet->ztscTracker.get();
    CMintMeta meta;
    if (!ztscTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    ztscTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a TSC stake is the outpoint
