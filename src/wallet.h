...
enum ZerocoinSpendStatus {
    ...
    ZTSC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZTSC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZTSCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fztscAuto) const;
    ...
    */
    ...
    CzTSCWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzTSCTracker> ztscTracker;
    ...
    void setZWallet(CzTSCWallet* zwallet)
    {
        zwalletMain = zwallet;
        ztscTracker = std::unique_ptr<CzTSCTracker>(new CzTSCTracker(strWalletFile));
    }

    CzTSCWallet* getZWallet() { return zwalletMain; }
    ...
    /** zTSC reset */
    boost::signals2::signal<void()> NotifyzTSCReset;
