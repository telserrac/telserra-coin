...
public:
    ...
    bool WriteZTSCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZTSCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZTSCSeed_deprecated(uint256& seed);
    bool EraseZTSCSeed();
    bool EraseZTSCSeed_deprecated();

    bool WriteZTSCCount(const uint32_t& nCount);
    bool ReadZTSCCount(uint32_t& nCount);
