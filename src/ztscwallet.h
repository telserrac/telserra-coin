...
#ifndef TelSerraCoin_ZTSCWALLET_H
#define TelSerraCoin_ZTSCWALLET_H
...
public:
    CzTSCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZTSC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZTSC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //TelSerraCoin_ZTSCWALLET_H
